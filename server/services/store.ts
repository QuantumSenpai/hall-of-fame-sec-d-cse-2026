...
export const initialPhotos: Photo[] = [
  ...
  },
  {
    ...
    status: "published",
    displayOrder: 3,
    createdAt: new Date("2026-09-04T10:45:00Z"),
  },
  {
    id: 4,
    chapterId: 4,
  ...
  },
  {
    ...
    status: "published",
    displayOrder: 5,
    createdAt: new Date("2026-09-04T13:15:00Z"),
  },
  {
    id: 6,
    chapterId: 6,
    ...
    status: "published",
    displayOrder: 6,
    createdAt: new Date("2026-09-04T14:45:00Z"),
  },
  {
    id: 7,
    chapterId: 7,
    ...
    caption: "An afternoon we'll never forget — smiles that spoke louder than any speech.",
  ...
  },
  {
    ...
    status: "published",
    displayOrder: 9,
    createdAt: new Date("2026-09-04T23:45:00Z"),
  },
];
...
class MemoryStore {
  ...
  // Photos
  getPhotos(category?: string, chapterId?: number, includeUnpublished = false) {
    return this.photos
      .filter((p) => {
        if (!includeUnpublished && p.status !== 'published') return false;
        if (chapterId && p.chapterId !== chapterId) return false;
        if (category && category.toUpperCase() !== 'ALL') {
          return p.category && p.category.toUpperCase() === category.toUpperCase();
        }
        return true;
      })
      .sort((a, b) => a.displayOrder - b.displayOrder);
